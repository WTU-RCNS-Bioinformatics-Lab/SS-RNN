...
def plot_train_test_loss(params, pic_path, dataset):
    ...
    def _plot(params):
        ...
        fig.savefig(os.path.join(sub_pic_path, 'loss_'+model_name+'.jpg'), dpi=600)

    skip_a_params = params.query("model=='SkipAModel'")
    skip_a_params = skip_a_params.sort_values(['skip'])
    _plot(skip_a_params)

    skip_b_params = params.query("model=='SkipBModel'")
    skip_b_params = skip_b_params.sort_values(['skip'])
    _plot(skip_b_params)

    skip_c_params = params.query("model=='SkipCModel'")
    skip_c_params = skip_c_params.sort_values(['skip'])
    _plot(skip_c_params)

    skip_d_params = params.query("model=='SkipDModel'")
    skip_d_params = skip_d_params.sort_values(['skip'])
    _plot(skip_d_params)

    skip_e_params = params.query("model=='SkipEModel'")
...
def plot_violin(data, pic_path, dataset, title, label_list):
    ...
    if not os.path.exists(sub_pic_path):
        os.makedirs(sub_pic_path)
    plt.savefig(os.path.join(sub_pic_path, title.lower()+'_violin.jpg'), dpi=600)
...
def get_result_data(params):
    ...
    for i, row in params.iterrows():
        ...
        label = ''
        if model_name == 'NormalModel':
            label = label + 'Original'
        else:

            label = label + 'Model' + model_name[4] + '-' + skip
...
if __name__ == '__main__':
    ...
    pic_path = os.path.join(parent, 'pic')
    result_path = os.path.join(parent, 'result/release/all/')

    # plot_result(result_path, pic_path)
    params = get_param_from_path(result_path)



    for dataset in ['ecg','eeg','miteeg']:
